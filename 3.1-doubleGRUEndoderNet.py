...
class PlainEncoder(nn.Module):
    def __init__(self, vocab_size, hidden_size, dropout=0.2):
        ...
        self.rnn3 = nn.GRU(hidden_size, hidden_size, batch_first=True)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x, lengths):
        ...
        packed_embedded = nn.utils.rnn.pack_padded_sequence(embedded, sorted_len.long().cpu().data.numpy(),
                                                            batch_first=True)
        packed_out, hid = self.rnn(packed_embedded)
        packed_out2,hid2 = self.rnn2(packed_out)
        packed_out3, hid3 = self.rnn2(packed_out2)
        out, _ = nn.utils.rnn.pad_packed_sequence(packed_out3, batch_first=True)
        _, original_idx = sorted_idx.sort(0, descending=False)
        out = out[original_idx.long()].contiguous()
        hid = hid[:, original_idx.long()].contiguous()
