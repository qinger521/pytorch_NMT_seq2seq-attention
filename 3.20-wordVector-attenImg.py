...
def show_attention(attens):
    d = attens.detach().numpy()
    d = d[0]
    d = d.transpose()
    #col = [t for t in x]  # 需要显示的词
    #index = [t for t in y]  # 需要显示的词
    df = pd.DataFrame(d)
...
def translate_dev(i):
    ...
    print("".join(cn_sent))
    source.append([inv_cn_dict[w] for w in dev_cn[i]])
    ...
    #show_attention(attn)
    print("".join(trans))
    print("BLEU score:%f",sentence_bleu(source,trans))
    BLEU.append(sentence_bleu(source,trans))



torch.save(model.state_dict(), 'pth/model1.pth')
# torch.save(model, 'pth/model1.pth')
...
plt.subplot(2, 1, 2)
plt.plot(x, y, '.-')
plt.xlabel('Test BLEU vs. each Sentence')
